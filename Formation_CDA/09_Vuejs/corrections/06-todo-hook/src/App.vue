...
onBeforeMount(() => {
  if ('todos' in localStorage) {
    ...
    // solution intéressante lorsque on ne maîtrise pas le cycle de vie du composant
    // window.addEventListener('beforeunload', () => localStorage.setItem('todos', JSON.stringify(todos.value)));
  }
});

// Ce code n'est pas optimisé car il met à jour le local storage lorsque n'importe quelle variable réactive est modifiée
onUpdated(() => {
